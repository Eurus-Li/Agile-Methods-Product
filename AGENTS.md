# AGENTS.md — Rongrong 项目规则

唯一的项目规则文件。**任何 AI 编码助手（Claude Code / Codex / 其他）和人类贡献者，开始任务前先读这份文件。** 更深的技术细节看 [docs/architecture.md](docs/architecture.md)，视觉规范看 [docs/design-system.md](docs/design-system.md)，历史决策看 [docs/decisions.md](docs/decisions.md)，具体某个功能看 [docs/specs/](docs/specs/)。

## 项目是什么

Rongrong 是一款心理健康陪伴类产品：养一只虚拟小伙伴，记录每日心情、写 Journal、给 Rongrong 换装、有一个模拟的 Plus 会员方案。现状：一个交互逻辑已经跑通的 web demo（[web/](web/)）。下一步：在 [ios/](ios/) 做成 iOS 原生 App。**iOS 体验优先，暂不做 Android / 跨平台。**

`web/` 和 `ios/` 是同一套产品逻辑的两种实现，不是两个产品——改产品逻辑/文案先改 spec，两边实现都要跟着更新；只改某一端的实现细节，不动 spec。

[`prototypes/plus-standalone/`](prototypes/plus-standalone/README.md) 是更早期的一个独立付费墙原型，已经被 `web/` 里内置的 Plus 流程取代，保留只是为了参考它的测试用例，**不要在这上面继续开发新功能**。

另外还有一套完全独立的实现在 `feature/pip-frontend` 分支（未合并到 `main`，另一位作者独立开发中）。**先不要动那个分支**，架构怎么整合是待定事项，等确认过再处理，不要自作主张合并或参考它改这里的代码。

## 技术栈速览

- **`web/`（现有）：** 原生 HTML/CSS/JS，零依赖、零构建，`cd web && npm run dev` 直接跑。
- **`ios/`（规划中，代码尚未开始）：** Swift + SwiftUI + MVVM + SwiftData 本地持久化；无后端；不引入依赖除非必要。
- 完整技术栈、模块划分、数据模型约定 → [docs/architecture.md](docs/architecture.md)。
- 颜色/字体/间距/组件 Token → [docs/design-system.md](docs/design-system.md)，不允许现场发明新数值。
- 已经拍板 / 明确延后的选型（后端、支付、暗色模式等）→ [docs/decisions.md](docs/decisions.md)，不要重新讨论已经定过的事。

## 仓库结构

```
AGENTS.md / CLAUDE.md       # 本文件（唯一规则源）
README.md                   # 人类友好的项目介绍 + 怎么跑
docs/
  design-system.md            # 颜色/字体/间距/组件 Token
...
prototypes/
  plus-standalone/             # 早期付费墙原型，已被 web/ 取代，仅供参考
.github/                     # CI + PR 模板
```
...
3. **UI 相关改动**：颜色 / 圆角 / 字号从 `docs/design-system.md` 的 Token 表取值，不要现场发明新数值。
...
- 改动大、或会碰到多人共用的文件（数据模型、设计 Token、路由/导航层）时，除了开分支，最好再在团队渠道说一声，降低并行 AI 生成代码在同一批文件上互相冲突的概率。
...
- 不在 `prototypes/plus-standalone/` 上加新功能；不动 `feature/pip-frontend` 分支。
