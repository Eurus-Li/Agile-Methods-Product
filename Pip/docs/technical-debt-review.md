...
| **TD-2** | Whole `src/`, contrast with [`Commercialization/tests/`](../../Commercialization/tests/commercialization.test.js) | `streak()`, `level()`, `mostFrequent()`, `dateKey()`, and the load-time state merge/validation logic (`app.js:21-29`) are pure functions with **zero unit tests** | The sibling Commercialization project already proves this class of logic deserves tests (7 cases covering date boundaries/corrupt data) — this one has none | Month/year-boundary bugs, DST edge cases, or the "streak counts from yesterday if today has no entry" logic can silently break with no regression signal | Extract these pure functions (exportable once modularized) and cover them with `node --test`, mirroring the existing Commercialization test pattern | **High** | 5–7h |
...
- **Impact:** Prevents silent regressions in date-boundary logic (month/year rollover, DST); brings this project in line with the testing standard already set by the Commercialization project.
- **Remediation:** Mirror [`Commercialization/tests/commercialization.test.js`](../../Commercialization/tests/commercialization.test.js) using `node --test`; export the pure functions if needed.
- **Severity:** High
- **Effort:** 5–7h
